...
class CsvProductController extends Controller
{
    ...
    public function export()
    {
        ...
        foreach ($lists->toArray() as $list) {
            $list['photo'] = $resolveImageUrl($list['photo'] ?? '');
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            while (($line = fgetcsv($file)) !== false) {
                // Export has 35 columns (0–34); short rows must not trigger undefined index notices.
                $line = array_pad($line, 35, '');
                ...
                if ($i != 1) {
                    ...
                    $brand_id = $line[34] ? (Brand::whereName($line[34])->exists() ? Brand::whereName($line[34])->first()->id : 0) : 0;
