...
class CheckoutShippingHelper
{
    ...
    {
        return self::resolveSelectedOption($shippingId);
    }

    public static function billingToShippingPayload($request)
    {
        return [
    ...
    public static function orderShippingColumns($shipping)
    {
        if (!$shipping) {
            return [
        ...
        $meta = [
            ...
            'service_id' => $shipping['service_id'] ?? null,
            'rate_id' => $shipping['rate_id'] ?? null,
            'service_name' => $shipping['service_name'] ?? null,
            'carrier_name' => $shipping['carrier_name'] ?? null,
            'transit_days' => $shipping['transit_days'] ?? null,
            'quote_uuid' => $shipping['quote_uuid'] ?? null,
            'currency' => $shipping['currency'] ?? 'CAD',
            'remarks' => $shipping['remarks'] ?? null,
            'price' => $shipping['price'] ?? 0,
            'title' => $shipping['title'] ?? null,
        ];

        return [
            'shipment_provider' => $shipping['source'] ?? ($shipping['provider'] ?? 'internal'),
            'shipping_method_code' => (string) ($shipping['rate_id'] ?? $shipping['service_id'] ?? $shipping['id'] ?? ''),
            'shipping_method_name' => $shipping['title'] ?? $shipping['service_name'] ?? null,
            'shipping_carrier' => $shipping['carrier_name'] ?? null,
            'shipment_meta' => json_encode(['quote' => $meta]),
        ];
    }
    ...
    {
        if (empty($shippingAddress) || !self::isCanada($shippingAddress['ship_country'] ?? null)) {
            return false;
        }

        return trim((string) ($shippingAddress['ship_first_name'] ?? '')) !== ''
