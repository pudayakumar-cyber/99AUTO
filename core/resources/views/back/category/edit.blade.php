								...
								<form class="admin-form" action="{{ route('back.category.update',$category->id) }}"
									...
									<div class="card mb-4">
										<div class="card-body">
											...
											<p class="text-muted mb-3">{{ __('These category-level dimensions are used when a product does not have its own package data.') }}</p>
											<div class="row">
												<div class="col-md-3">
													<div class="form-group">
														<label for="package_length">{{ __('Length (in)') }}</label>
														<input type="number" step="0.01" min="0" name="package_length" class="form-control" id="package_length" value="{{ old('package_length', $category->package_length) }}">
													</div>
												</div>
												<div class="col-md-3">
													<div class="form-group">
														<label for="package_width">{{ __('Width (in)') }}</label>
														<input type="number" step="0.01" min="0" name="package_width" class="form-control" id="package_width" value="{{ old('package_width', $category->package_width) }}">
													</div>
												</div>
												<div class="col-md-3">
													<div class="form-group">
														<label for="package_height">{{ __('Height (in)') }}</label>
														<input type="number" step="0.01" min="0" name="package_height" class="form-control" id="package_height" value="{{ old('package_height', $category->package_height) }}">
													</div>
												</div>
												<div class="col-md-3">
													<div class="form-group">
														<label for="package_weight">{{ __('Weight (lb)') }}</label>
														<input type="number" step="0.01" min="0" name="package_weight" class="form-control" id="package_weight" value="{{ old('package_weight', $category->package_weight) }}">
													</div>
												</div>
											</div>
										</div>
