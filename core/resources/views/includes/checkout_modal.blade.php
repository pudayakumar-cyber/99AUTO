     ...
     <!-- Modal Stripe Payment Element (Link + card) — needs tall mount; never .form-control / fixed height -->
     <div class="modal fade" id="stripe" tabindex="-1" aria-hidden="true">
         <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
             <div class="modal-content">
                 <div class="modal-header">
                     <h6 class="modal-title">{{ __('Pay with Credit Card') }}</h6>
                     <button class="close" type="button" data-bs-dismiss="modal" aria-label="Close"><span
                             aria-hidden="true">&times;</span></button>
                 </div>
                 <div class="modal-body pb-0">
                     <form id="stripe-payment-form">
                         ...
                         <input type="hidden" name="shipping_id" value="" class="shipping_id_setup" id="stripe_shipping_id">
                         <input type="hidden" name="state_id"
                             value="{{ auth()->check() && auth()->user()->state_id ? auth()->user()->state_id : '' }}"
                             class="state_id_setup" id="stripe_state_id">

                         <p class="mb-3">{{ PriceHelper::GatewayText('stripe') }}</p>

                         <div class="form-group mb-0">
                             <label class="d-block mb-2" for="card-element">{{ __('Card Details') }}</label>
                             <div id="card-element" class="stripe-payment-element-mount w-100"></div>
                             <div id="card-errors" role="alert" class="text-danger small mt-2"></div>
                         </div>

                         <div id="stripe-payment-message" class="alert alert-danger d-none mt-3 mb-0"></div>
                     </form>
                 </div>
                 <div class="modal-footer flex-shrink-0 border-top bg-white">
                     <button class="btn btn-secondary btn-sm" type="button"
                         data-bs-dismiss="modal"><span>{{ __('Cancel') }}</span></button>
                     <button class="btn btn-primary btn-sm" type="button" id="stripe-submit-button">
                         <span id="stripe-button-text">{{ __('Pay Now') }}</span>
                         <span id="stripe-spinner" class="spinner-border spinner-border-sm d-none" role="status" aria-hidden="true"></span>
                     </button>
                 </div>
             </div>
         </div>
     ...
     <style>
         #stripe .stripe-payment-element-mount {
             min-height: 22rem;
